                ...
                {step === 3 && (
                    <div className="text-start relative mt-14  ">
                        ...
                        <div className="flex justify-between pt-4">
                            ...
                            <button
                                onClick={async () => {
                                    await handleClaimBusiness(); // Claim the business
                                    if (isBusinessClaimed) { // Check if the business was claimed
                                        handleNext(); // Move to the next step if claimed successfully
                                    }
                                }}
                                className={`btn-style w-1/4 rounded ${!isConnected ? "opacity-50 cursor-not-allowed" : ""}`}
                                disabled={!isConnected} // Disable only if not connected
                            >
                                Next
                            </button>
                        </div>
...
const FormifyProject = ({ walletAddress, onBusinessClaimed }) => {
    ...
    const [isBusinessClaimed, setIsBusinessClaimed] = useState(false);
    const [loading, setLoading] = useState(false);
    const [storeId, setStoreId] = useState(null);
    const { address, isConnected, chain } = useAccount();
    const { disconnect } = useDisconnect(); // Use useDisconnect hook
